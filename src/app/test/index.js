    ...
    <>
      <div className="lg:w-full xs:w-fit max-w-[1824px] m-auto grid grid-cols-1 gap-10 justify-center items-center bg-white">
          ...
          <div className="w-10/12 m-auto flex flex-col items-start gap-5">
            <div className="w-full flex justify-between">
              ...
              <div className="w-fit flex gap-2 justify-between">
                <button className="b3-button cursor-pointer">
                  <FiChevronLeft />
                </button>
                <button className="b3-button dcursor-pointer">
                  <FiChevronRight />{' '}
                </button>
              </div>
            </div>
            <div className="w-full flex gap-8 overflow-x-scroll">
              <Card2
                title={'Title'}
                plot={
                  'one of the greates series that you will ever see, trust me, it works'
                }
                img={'/images/hotd.jpg'}
                url={'URL'}
              />
        ...
        <div className="w-full p-10 flex gap-20 justify-center items-center foreground2">
          TEST HERE
          <button className="p-5 rounded-md bg-red-500" onClick={getData}>
            GET DATA
          </button>
          <Link className="p-5 rounded-md bg-blue-500" href={'/'}>
            Home
          </Link>
          <div>
            {data.map((item, index) => (
              <div key={index}>
                ...
                <img
                  className="w-40 p-5 bg-sky-500"
                  src={item.node.entity.primaryImage.url}
                />
              </div>
            ))}
          </div>
